...
with open('datosia.json') as file:
    datav = json.load(file)

with open('datosip.json') as file:
...
def SendData(mycursor, mydb):
     ...
     #resultadov = rv.json()
     viento = (datav['observations'][i]['metric']['windspeedAvg'])*(10/36) #Se obtiene el viento   
     ...
     #resultador = rr.json()
     radiacion = datar['observations'][i]['solarRadiationHigh'] #Se obtiene la radiacion 
     fechayhora = datar['observations'][i]['obsTimeLocal'] #Obtengo la hora
     fechayhora = datetime.strptime(fechayhora, '%Y-%m-%d %H:%M:%S')
...
while True: 
     ...
     if (tiempo >= 10): #Cada 5 minutos se envía un dato a la BD
        print("Sending")   
